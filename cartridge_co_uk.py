            ...
            for li in section.find_all("li", class_='product-item'):
                for a in li.find_all("a", class_='product-item-link'):
                    if "Genuine" in a.text and a["href"] not in rows_written: 
                        type_raw = section.find_all('div', class_='group-name')[0]
                        type_conso_full = "type_conso : " + type_raw.text
                        type_conso = "type_conso : " + type_raw.find_all('strong')[0].text
                        url = "url : " + a["href"]
                        img = "img : " + li.find_all('a', class_="product-item-photo")[0]['data-image-src'] #they lazy load so taking the data-src one
